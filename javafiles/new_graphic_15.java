...
public 	class 	new_graphic_15
	...
	public static void main(String args[]) {
		...
		new_graphic_15 a = new new_graphic_15();
	...
	public new_graphic_15() {
			...
			//INTERFACE
		{
			...
			filename	= new TextField("test.txt", 15);	//file to save
			note		= new TextArea(5,18);
			
			// --- Buttons --------------------------------
			start 		= new Button("Start NEW");
			startOld	= new Button("Start AN.");
	...
	public void paint(Graphics g) {
		...
		if (startmovie == false) {
			...
			if (savemaskpng==1) {
				...
				// Take folder name as file name to save the mask
				...
				for (int i = 0; i < filNamLen-1; i++) {
					...
					if (strTock.equals(findSl) && found == 0) {
						indexToTake = i;
						found = 1;
						//System.out.println("index "+indexToTake+" strTock "+strTock);
					}
				}
				String fileName0 = namefilel.substring(filNamLen-indexToTake, filNamLen-4);
				//System.out.println("index "+indexToTake+" length "+fileName0);
				String maskfilenamepng = String.valueOf("mask/"+"mask_"+fileName0+".png");
			...
			if (diSpacec) {
				...
				if (savedipng==1) {
					// Take folder name as file name to save the DI
					int filNamLen = namefilel.length();
					int indexToTake = 5;
					int found = 0;
					String findSl = "/";
					...
					String fileName0 = namefilel.substring(filNamLen-indexToTake, filNamLen-4);
					String apdfilenamepng = String.valueOf("apddiSpace/"+"DI_"+fileName0+"_nfr_"+napddi+".png");
			...
			if (apdSpacec) {
				...
				if (saveapdpng==1) {
					// Take folder name as file name to save the APD
					int filNamLen = namefilel.length();
					int indexToTake = 5;
					int found = 0;
					String findSl = "/";
					...
					String fileName0 = namefilel.substring(filNamLen-indexToTake, filNamLen-4);
					String apdfilenamepng = String.valueOf("apddiSpace/"+"APD_"+fileName0+"_nfr_"+napddi+".png");
		...
		// *** Plot movie ***
			...
			if (nmap == 0) {// Full scale
				...
				for (int i = 0; i < ss; i++) {
					for (int j = 0; j < ss; j++) {
						int iii = 0;
							//NNNEW to show the structure under the activation
	...
	//*************************************************************
		...
		if (ae.getSource() == startOld && startOld.getLabel().equals("Start AN.")) {
		...
		if (ae.getSource() == save) {
			...
			note.append("\nDIM: "+u.length+", "+u[0].length+", "+u[0][0].length);
			try {
	...
	/*
	// This method reads ANALYZED data and give matrix dimensions
		public short[][][] readAnData(String namefilel) { 
			try{
			//	short [][][] u; //= data.readVideoFile(namefilel);
				//nframes = data.readNumbFrame();
				//sizex = data.readXDim();
				//sizey = data.readYDim();
				//return u;
	...
	public static void spaceAverage( short[][][] u,  short[][] mask, 
		...
		for (int i = 0; i <= 12; i++){
			for (int j = 0; j <= 12; j++){
				int ii = i - 6;
				int jj = j - 6;
				xcoef[i][j] = 1000*9*Math.exp(-( (ii*ii) + (jj*jj) )/6);
			}
		}
		note.append( "\nAveraging....");
		kt=0;
