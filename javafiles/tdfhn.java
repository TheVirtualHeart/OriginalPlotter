...
//*******************************************************************************
      ...
      //import java.awt.image.ColorModel;
        import java.awt.image.IndexColorModel;
         ...
         /*
         <applet code="tdfhn" width=490 height=500>
          </applet>
          */
     ...
     public class tdfhn extends Applet
      ...
      Thread  thread;
      Button bstartstop, bric,bwic,brestart, breseth, btip, bsignal, bstim, bvf;
      ...
      int gsize=200,s1size=10,gsizeold=200;
      ...
      int ndef=0;
      int nnsin=400;
      int k=0,ktime,nstop=0,nbsignal=0,nbstim=1,nok=0,nok1=0,nstart=0,msenal=0;
...
//              Create Buttons
      bvf = new Button ("Initiate VF");
      bstartstop = new Button ("Pause");

      bric       = new Button ("Initiate VT");
      bwic       = new Button ("Normal SN");
      brestart   = new Button ("Defibrillation Shock");
...
bstartstop.reshape (2, 2, 70, 25);
bvf.reshape(74,2,100,25); 
bric.reshape (176,2,100,25);
bwic.reshape (410,2,80,25);
brestart.reshape (278,2,130,25);
breseth.reshape (935,2,55,25);
//btip.reshape (283,2, 55, 25); 
//bsignal.reshape (380, 300, 60, 25);
bstim.reshape (992, 150, 50, 25);
...
ls1size = new Label("Stimulus size");
...
// checkboxes
        ...
        public void mousePressed(MouseEvent me){
         ...
         iipx=(int)((float)(p.x-15)/(1.5*1.15));
         iipy=(int)((float)(p.y-200)/1.5);
             nok1=0;
          if(200-iipx > 0 && 200-iipy > 0 && iipx >= 0 && iipy >=0){
             ipx=(int)((float)(p.x-15)/(1.5*1.15));
             ipy=(int)((float)(p.y-200)/1.5);
             nok1=1;
             nxsignal=0;
             for(int i=0; i<90; i++){
                usig[i]=0;
...
//------------------end of bvf
           ...
           if(ae.getSource()==bstartstop){
             if(bstartstop.getLabel().equals("Continue")){
...
// ---- DD calcualte tip
  ...
  //      g.fillRect(240,345,70,20);
       ...
       if(mplot==1){
           ...
           g.drawString("Stimulus Size",383,214);
                        } 
           else{
