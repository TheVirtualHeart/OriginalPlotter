...
public 	class 	new_graphic_17
	...
	// APD - DI mean values ----------------------------------------------------
	int 	meandi[][], meanapd[][];// No alternance
	short 	minDIAe[][],maxDIAe[][],minAPDAe[][],maxAPDAe[][];// Presence of alternance
	short 	minDIAo[][],maxDIAo[][],minAPDAo[][],maxAPDAo[][];// Presence of alternance
	int 	mdiAo[][],mapdAo[][],mdiAe[][],mapdAe[][];// Presence of alternance
	//Movie declarations -------------------------------------------------------
	Thread thread;
	...
	public static void main(String args[]) {
		...
		new_graphic_17 a = new new_graphic_17();
		
	}//close main
	//**************************************************************************	
	//This method builts the graphical interface		
	public new_graphic_17() {
	...
	//**************************************************************************
		...
		if (ae.getSource() == startOld && startOld.getLabel().equals("Start Old")) {
			...
			if (ae.getSource() == startOld && startOld.getLabel().equals("Reset")) {
				...
				ormax = null; ormin = null;
				ntll = null; intervsh1f = null;
				intervsh2p = null; apd = null;
			    di = null; tpb = null;
		...
		if (ae.getSource() == save) {
			...
			//datx = new DataText(namefilel,freq,ur,mmmdi,mMxdi,mmmapd,mMxapd); 
			//datx.addWindowListener(new WindowAdapter(){public void windowClosing(WindowEvent we) {datx = null;}}); 
		}
		...
		if (ae.getSource() == start && start.getLabel().equals("Start New")) {
			...
			short[][][] zero3D = new short[sizex][sizey][nframes];
			for (int i = 0; i < sizex; i++){
				zero1D[i] = 0;
				for (int j = 0; j < sizey; j++){
					zero2D[i][j] = 0;
			...
			// Initialize space and time dimensions ----------------------------
			ntimes = nframes;
			nii = sizex/2;
			njj = sizey/2;
			...
			ormax = maxtest; ormin = mintest;
			min = mintest; max = maxtest;
		...
		if (ae.getSource() == setshift) {
			...
			*/
			...
			shifted = tempshifted;
			basevL = tempbasevL;
			intervsh1f = intervsh1first;
			intervsh2p = intervsh2final;
			// Max min
	...
	public static void minMax( short[][][] signal, short[][] mask,
							   int sizex, int sizey, int ntimes,
							   short[][] max, short[][] min){
	...
	public short [][][] timeAverage(short[][][] u, short[][] mask,
									int sizex, int sizey, int ntimes){
	...
	public static void shiftSignal( short[][][] u, short[][] mask,
								    ...
								    short[][][] nnso, 
								    short[][][] shifted,
								    short[][] intervsh1first, short[][] intervsh2final ){
