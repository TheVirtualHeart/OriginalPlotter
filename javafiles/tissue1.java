          /*
         <applet code="tissue1" width=600 height=400>
         </applet>
          */
...
public class tissue1 extends Applet
...
/*-------------------------------------- define gloval -----*/
  ...
  int NN = Nmt + 1;
  double t = 0.0;               /* time (s)               */
  double dt = 0.0001;           /* time division (s)      */
...
/*------------------------ set initial position   ---------*/
  ...
  int matter(double x, double y) {
    int mt;

    mt = -1;
    if ((x-0.16)*(x-0.16)+(y-0.086)*(y-0.086)<0.082*0.082) {
      mt = 0;
    } else if ((x-0.4)*(x-0.4)+(y-0.052)*(y-0.052)<0.041*0.041) {
      mt = 0;
    } else if (x>0.0 && x<0.6 && y>0.28 && y<0.40) {
      mt = 0;
    }
    return (mt);
  }
