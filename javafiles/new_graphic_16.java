 ...
 */
 ...
 /*
 To compile it needs:
 - DIRECTORIES: "/movie", "/mask", "/analyzed"
...
public 	class 	new_graphic_16
	...
	private TextField 	tnii, tnjj, tfreq, erasesx, erasedx, thr;
	...
	boolean beginAnalyzeD = false;		//start analysis for analyzed file
	...
	TextField displayFilename = new TextField(30);
	...
	public static void main(String args[]) {
		...
		new_graphic_16 a = new new_graphic_16();
	...
	public void paint(Graphics g) {
		...
		if (startmovie == false) {
			...
			// Original -> signal1
			if (originc && beginAnalyzeD==false) {
			...
			// Unwrapped -> signal2
			if (unwrapc && beginAnalyzeD==false) {
			...
			if (frame11 && beginAnalyzeD==false) {
			...
			if (diSpacec) {
				...
				gBuffer.setFont(new Font("Serif", Font.BOLD, 15));
				gBuffer.drawString("DI: ", dsx+pw-ss-35, dsy-3);
				gBuffer.drawString(String.valueOf(di[nii][njj][napddi])+" ms", dsx+pw-ss, dsy-3);
				gBuffer.setFont(new Font("Serif", Font.PLAIN, 16));
				// Plot DI in space
				...
				if (savedipng==1) {
					String apdfilenamepng = String.valueOf("apddiSpace/"+"DI_"+analyzedfile+"_nfr_"+napddi+".png");
			...
			if (apdSpacec) {
				...
				gBuffer.setFont(new Font("Serif", Font.BOLD, 15));
				gBuffer.drawString("APD: ", dsx+pw-ss-35, dsy-3);
				gBuffer.drawString(String.valueOf(apd[nii][njj][napddi])+" ms", dsx+pw-ss+5, dsy-3);
				gBuffer.setFont(new Font("Serif", Font.PLAIN, 16));
				for (int ii = 0; ii < (maplength - 1); ii += 1){
					int iii = ii;
					if (iii < 30) iii = 30;
	...
	//*************************************************************
		...
		if (ae.getSource() == openButton) {
			if (start.getLabel().equals("Start New") || startOld.getLabel().equals("Start Old")) {
				...
				startOld.setLabel("Start Old");
				startOld.setBackground(Color.red); 
			}
		}
		...
		// Start to plot the analyzed data
		if (ae.getSource() == startOld && startOld.getLabel().equals("Start Old")) {
			...
			startOld.setLabel("Reset");
			startOld.setBackground(Color.gray);
			...
			nii = (int)(nnii);
			njj = (int)(nnjj);
			note.append("\nLoading analyzed file. \n");
			try {
				DataInputStream in = new DataInputStream(new BufferedInputStream(new FileInputStream(namefilel)));
				for (int i = 0; i < 128; i++) {
					for (int j = 0; j < 128; j++) {
						for (int k = 0; k < 5000; k++) {
							u[i][j][k] = in.readShort();
			...
			note.append("Start plotting. \n");
		}
		else {
			if (ae.getSource() == startOld && startOld.getLabel().equals("Reset")) {
					// Reset matrices
				u = null; mask = null;
				video = null; uumovie = null;
				umovie = null; original = null;
				originalimg = null; unwrapped = null;
				ounwrapped = null; taveraged = null;
				shifted = null; basev1 = null;
				spaceaveraged = null; erased = null;
				max = null; min = null;
				maxINT = null; minINT = null;
				maMa = null; maMaApdDi = null;
				maxAInt = null; minAInt = null;
				omax = null; omin = null;
				ntll = null; intervsh1 = null;
				intervsh2 = null; apd = null;
			    di = null; tpb = null;
				meandi = null; meanapd = null;
				mdiAo = null; mapdAo = null;
				mdiAe = null; mapdAe = null;
				minDIn = null; maxDIn = null;
				minAPDn = null; maxAPDn = null;
				minDIAe = null;	maxDIAe = null;
				minAPDAe = null; maxAPDAe = null;
				minDIAo = null; maxDIAo = null;
				minAPDAo = null; maxAPDAo = null;
				nb = null; ntr = null;
				...
				apdDistc = false; alternancec	= false;	
				apdDiMeanc = false; apdSpacec = false;	
				diSpacec = false; frame11	= false;	
				frame1p	= false; 
					// Reset to plot
				mintimeplot = 10;
				...
				savemaskpng=0; savedipng=0; saveapdpng=0;
				namefilel=" "; sigsig=" "; oldText=" ";
					// Dimension Parameters
				nf=128; ntimes=5000; nii=60; njj=60; ur=2500; norma=10000;				
					// To normalize the plot
		...
		if (ae.getSource() == save) {
			...
			anfiletosave = String.valueOf("analyzed/"+"AN_"+analyzedfile);
			try {
		...
		if (ae.getSource() == start && start.getLabel().equals("Start New")) {
			...
			note.append( "\nTime averaging.... ");
//NNNNNNNNEW
			spaceAverage(u, mask, sizex, sizey, ntimes, nii, njj, oldText, note, u);
		...
		//	normaSignal(spaceaveraged, mask, sizex, sizey, ntimes, norma, max, min, spaceaveraged);
		...
		//	normaSignalGN(spaceaveraged, mask, sizex, sizey, ntimes, norma, max, min);
			...
			minMaxN(u, mask, sizex, sizey, ntimes, max, min, spaceaveraged);			
               //         spaceaveraged=u;
			u = spaceaveraged;
	...
	// This method evaluates the maximum and minimum values
	public static void minMaxN( short[][][] u, short[][] mask,
							   int sizex, int sizey, int ntimes, 
							   short[][] max, short[][] min, short[][][] spaceaveraged ){
		for (int i = 0; i < sizex; i++){
			for (int j = 0; j < sizey; j++){			
				if (mask[i+6][j+6] == 1) {
					max[i][j] = -32768;
					min[i][j] = 32767;	
					for (int k = 10; k < (ntimes - 10); k++) {
                                                    spaceaveraged[i][j][k]= (short)(u[i][j][k]*1.5-900);
                                                  
                                                if(spaceaveraged[i][j][k] > 9000)spaceaveraged[i][j][k]=9000; 
                                                if(k>5060)spaceaveraged[i][j][k]=(short)(spaceaveraged[i][j][k]*.3);
					//	if (u[i][j][k] < min[i][j]) min[i][j] = u[i][j][k];
					//	if (u[i][j][k] > max[i][j]) max[i][j] = u[i][j][k];
					}
				}
					...
					if ((max[i][j] - min[i][j]) == 0) 
						...
						for (int k = 0; k < (ntimes - 0); k++){
							...
							various[i][j][k] = u[i][j][k];
                                          //             if(various[i][j][k]>10000)various[i][j][k]=9500;
                                          //        if(k>2820){various[i][j][k]= (short)(various[i][j][k]*.9);}
						}
					}
				}
			}
	...
	public static void spaceAverage( short[][][] u,  short[][] mask, 
		...
		for (int i = 0; i <= 12; i++){
			for (int j = 0; j <= 12; j++){
				int ii = i - 6;
				int jj = j - 6;
				xcoef[i][j] = 1000*9*Math.exp(-( (ii*ii) + (jj*jj) )/3);
			}
		}
		note.append( "\nAveraging....");
		kt=0;
...
6:55
6:55pm, 7:17pm, 8:17pm, 8:55pm, 10:02pm
8:17pm-10:00pm, 8:55pm-10:44pm
[12:28:25 PM] elizabeth.m.cherry: 8:17 from platform 10, ICE 673
[12:28:38 PM] elizabeth.m.cherry: 8:55 from Platform 10, ICE 877
