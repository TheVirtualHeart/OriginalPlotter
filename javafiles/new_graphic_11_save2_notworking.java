...
import java.lang.*;
import java.text.*;
import java.io.*;        //NNEW added for the gif files
import javax.imageio.*;  //NNEW added for the gif files
import java.awt.image.*; //NNEW added for the gif files
...
public 	class 	new_graphic_11
	...
	private Button		start, maskb, pause, movie; //NEW Stop Changed
        private Button          savemoviepng;  //NNEW added to save png files
	...
	public new_graphic_11() {
			...
			//INTERFACE
		{
			...
			movie 		= new Button("Movie");
                    savemoviepng           = new Button("Save Movie");  //NNEW button for png files
			...
			movie.addActionListener(this);
                    savemoviepng.addActionListener(this); //NNEW  for saving png files
				...
				//Elements Position
			{
				...
				rowW.add	( rowWfpm );
                                rowW.add        ( savemoviepng );  //NNEW added button for png files
	...
	public void paint(Graphics g) {
          ...
          //NNEW bi and g2 below are defined to save images png's for movies later on
				...
				if (nmap == 0) {
					...
					for (int i = 0; i < ss; i++) {
						for (int j = 0; j < ss; j++) {
							...
							gBuffer.setColor(new Color(ir[iii],ig[iii],ib[iii]));
							gBuffer.fillRect(dsx+i*2 , dsy+j*2 , 2, 2);	
           //NNEW g2 added to save png files for movies
       ...
       //NNEW  added to save png files on the movie directory
       // Remember to create a movie directory before running the program!


//             if(savemovieframes==1){
/*
          if(time < 10){
        String filename = String.valueOf("movie/000"+time+".png");

                       }
          if(time >= 10  && time < 100){
        String filename = String.valueOf("movie/00"+time+".png");

                       }

          if(time >= 100 && time < 1000){
        String filename = String.valueOf("movie/0"+time+".png");

                       }
          if(time >= 1000 ){
        String filename = String.valueOf("movie/"+time+".png");
