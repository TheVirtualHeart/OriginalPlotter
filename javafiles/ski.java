...
/*      <applet code="ski" width=350 height=280>
         </applet>
*/

	public class ski extends Applet
	implements MouseListener, ActionListener, Runnable{
        Point p;
        Button butt,butf,buts,butr,butrs,butla,butsm,butcont;
        int nx=200;
        int nstop=1;
        int nspeed=20;
        double pp;
//        int angle=0;
	...
	double di_aver=0, di_time=0, ncontrol=0, nfront=0;
	double diprev=0;
	double apdprev=0;
	double downtime=0;
	double uptime=0;
...
//                               }
	...
	butr = new Button ("Reset");
        ...
        add (butsm);
	butcont= new Button ("control");
	butcont.addActionListener(this);
	add (butcont);
	}
...
//                                                      }
         ...
         if(ae.getSource()==butcont){
//	      di_aver=0;
//	      di_time=0;
//	      nfront=0;
	      ncontrol=ncontrol+1;
	      if(ncontrol >= 2)ncontrol=0;
	                            };
                                                         }
...
//        g.drawString("points",119,275);
         ...
         g.drawLine(i+20,yy1,i+21,yy2);
                        ...
                        g.drawString(String.valueOf((DI1)),250,126);
                         g.drawString(String.valueOf((DI2)),250,146);
                         g.drawString(String.valueOf((ndown)),250,166);
                         g.drawString(String.valueOf((dig/nq)),250,186);

         if(ncontrol==1){g.drawString("Control on",250,50);
 			 
		      //     g.drawString(String.valueOf((u[100])),250,126);
			 }
         if(ncontrol==0)g.drawString("Control off",250,50);
...
//	      if(nfront == 1){
           ...
           if((u[100]-thresh)*(ut[100]-thresh)< 0){
           if(ut[100] < u[100]){

//	         if(u[100]>thresh && ut[100]<=thresh){ 
		 ndown=1;                            }
		 else {
		 ndown=0;
		 DI1=DI2;
		 ...
		 dig=dig+DI1;
                 ...
                 if(ndown==1){
	//	 downtime=downtime+(thresh-u[100])/(ut[100]-u[100])*dt;
	         downtime=downtime+dt;
		 if(ncontrol==1){
//		 if(DI2>DI1){
                 if(downtime > 85)xistim=750;
