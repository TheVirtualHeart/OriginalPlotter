...
public 	class 	new_graphic_12
	...
	int dpx		= 70;		//Distance from x window boundary
	...
	Image Buffer;
	Image bi, bimask; // To save movie frames and mask
	Graphics gBuffer;
	Graphics g2, g3; // To save movie frames and mask
	Label prompthoriz;
	...
	public void paint(Graphics g) {
		...
		if (startmovie == false) {
			...
			gBuffer.setColor(Color.black);
			//gBuffer.drawString("APDmax", dsx+2*ss+100, dsy+2*ss-maplength-19);
			//gBuffer.drawString("APDmin", dsx+2*ss+100, dsy+2*ss);
			...
			if (diSpacec) {
				gBuffer.setColor(Color.black);
				setFont(new Font("Serif", Font.PLAIN, 16));
				gBuffer.drawString("DImax", dsx+2*ss+95, dsy+2*ss-maplength-19);
				gBuffer.drawString("DImin", dsx+2*ss+95, dsy+2*ss);
				for (int ii = 0; ii < maplength; ii += 1){
					gBuffer.setColor(new Color(irr[ii],igg[ii],ibb[ii]));
					gBuffer.fillRect(dsx+2*ss+115, dsy+2*ss-16-ii, 10, 1);
				}
				if (alternancec) {
				}
				else {
					for (int i = 0; i < ss; i++) {
						for (int j = 0; j < ss; j++) {
							int iii=(int)((maplength*di[i][j][napddi]-maMaApdDi[0])/(maMaApdDi[1]-maMaApdDi[0]));
			...
			if (apdSpacec) {
				gBuffer.setColor(Color.black);
				setFont(new Font("Serif", Font.PLAIN, 16));
				gBuffer.drawString("APDmax", dsx+2*ss+95, dsy+2*ss-maplength-19);
				gBuffer.drawString("APDmin", dsx+2*ss+95, dsy+2*ss);
				for (int ii = 0; ii < maplength; ii += 1){
					gBuffer.setColor(new Color(irr[ii],igg[ii],ibb[ii]));
					gBuffer.fillRect(dsx+2*ss+115, dsy+2*ss-16-ii, 10, 1);
				}
				if (alternancec) {
					for (int i = 0; i < ss; i++) {
						for (int j = 0; j < ss; j++) {
							int iii=(int)(255*(apd[i][j][napddi]))/(maMaApdDi[3]-maMaApdDi[2]);
							if (iii > 255) iii = 0;
							if (iii < 0) iii = 0;
							gBuffer.setColor(new Color(irrr[iii], iggg[iii], ibbb[iii]));
							gBuffer.fillRect(dsx+pw-2*ss+i*2 , dsy+j*2 , 2, 2);
					...
					for (int i = 0; i < ss; i++) {
						for (int j = 0; j < ss; j++) {
							int iii=(int)((maplength*apd[i][j][napddi]-maMaApdDi[2])/(maMaApdDi[3]-maMaApdDi[2]));
							//if (iii > maplength) iii = maplength;
							//if (iii < 0) iii = 0;
							gBuffer.setColor(new Color(irr[iii],igg[iii],ibb[iii]));
							gBuffer.fillRect(dsx+pw-2*ss+i*2 , dsy+j*2, 2, 2);
		...
		// *** Plot movie ***
			...
			for (int i = 0; i < ss; i++) {
				for (int j = 0; j < ss; j++) {
					int iii=(int)(190*mask[i+6][j+6]);
					if (iii > 190) iii=190;
					if (iii < 30) iii=30;
					g3.setColor(new Color(irrr[iii],iggg[iii],ibbb[iii]));
