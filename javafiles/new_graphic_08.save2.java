...
public 	class 	new_graphic_08
	extends	Frame              //NEW added MouseMotionListener
	implements MouseListener,  MouseMotionListener, ActionListener, ItemListener, AdjustmentListener, Runnable {
	
	//Interface declarations
	private Button		start, maskb, pause, movie;     //NEW Changed
...
//	Color	background		= new Color(192,192,192);
	...
	int mindi1=10, maxdi1=100, minapd1=20, maxapd1=100; //APD - DI restitution up to 100 ms DI
	int mindi2=10, maxdi2=200, minapd2=20, maxapd2=200; //APD - DI restitution up to 200 ms DI
	int mindi3=10, maxdi3=300, minapd3=20, maxapd3=300; //APD - DI restitution up to 300 ms DI
	int mindi4=10, maxdi4=400, minapd4=20, maxapd4=400; //APD - DI restitution up to 400 ms DI
	int mindi5=10, maxdi5=500, minapd5=20, maxapd5=500; //APD - DI restitution up to 500 ms DI
	int clMaxc = 0;
	...
	short mask[][] = new short[128+12][128+12];   //NEW, moved from space average section 
	...
	public static void main(String args[]) {
		...
		new_graphic_08 a = new new_graphic_08();
	...
	public void paint(Graphics g) {
		...
		//	gBuffer.drawString(Integer.toString(imask), dpx + pw - dax - dax/2, dpy + phG - day/2);

				for (int i = 0; i < ss; i++) {
					for (int j = 0; j < ss; j++) {
						//int iii=(int)(255*(original[i][j][ftime]+32768))/(65535);
						//int iii=(int)(255*(original[i][j][ftime]))/(20000);
                                               // if(mask[i+6][j+6] == 0)original[i][j][ftime]=0;
				...
				/*for (int i = 0; i < ss; i++) {
					for (int j = 0; j < ss; j++) {
						int iii=(int)(190*umovie[i][j][ftime]/norma);
						if (iii > 190) iii = 190;
						if (iii < 0) iii = 0;
						gBuffer.setColor(new Color(irrr[iii], iggg[iii], ibbb[iii]));
						gBuffer.fillRect(dsx+i*2 , dsy+j*2 , 2, 2);
		...
		// *** Plot movie ***
			...
			if (time >= ntimes-2) {
				...
				repaint();
                   ...
                   if(mousezoom == 1){
                    gBuffer.setColor(Color.red);
                //    gBuffer.drawRect(nzoomx1-dpx, nzoomy1-dpy, (nzoomx2-nzoomy1)-dpx, (nzoomy2-nzoomy1)-dpy );
                    gBuffer.drawRect(nzoomx1, nzoomy1, nzoomx2-nzoomy1, nzoomy2-nzoomy1 );
	...
	//*************************************************************
		...
		if (ae.getSource() == start && start.getLabel().equals("Start")) {
			...
			// Call method to evaluate absolute Man and min --------------------
			note.append("\nAbsolute Max and min...");
			minAMaxA(sizex, sizey, ntimes, max, min, maMa);
			note.append("\nMaxA: "+maMa[1]); 
			note.append("\nMinA: "+maMa[0]+"\n");
			...
			for (int i = 0; i < sizex; i++) {
				for (int j = 0; j < sizey; j++) {
					for (int k = 10; k < (ntimes-10); k++) {
                      ...
                      // NEW  moved from space average section   // Check that this works ok for the CV strip case
		// Do Mask 
		for (int j = 0; j < (sizex + 12); j++){
			for (int i = 0; i < (sizey + 12); i++){
				// Mask
			...
			/*
			unwrapSignalINT(u,sizex,sizey,ntimes,uu);
			note.append("\nuu"+"["+1000+"]"+"["+nii+"]"+"["+njj+"]: "+uu[nii][njj][1000]); 
			note.append("\nuu"+"["+1100+"]"+"["+nii+"]"+"["+njj+"]: "+uu[nii][njj][1100]);
			
			minMaxINT(uu,sizex,sizey,ntimes,maxINT,minINT);
			note.append("\nMaxINT in "+"["+nii+"]"+"["+njj+"]: "+maxINT[nii][njj]); 
			note.append("\nMinINT in "+"["+nii+"]"+"["+njj+"]: "+minINT[nii][njj]);
			
			minAMaxAINT(sizex, sizey, ntimes, maxINT, minINT, maMa);
			note.append("\nMaxA: "+maMa[1]); 
			note.append("\nMinA: "+maMa[0]+"\n");
			*/
			//ounwrapped = u;
			//normaSignalGN(ounwrapped, sizex, sizey, ntimes, norma, max, min);
		...
		if (ae.getSource() == spaaverage) {
			note.append( "\nSpace average....");
                          //NEW added mask to the call
			spaceAverage(u, mask, sizex, sizey, ntimes, nii, njj, oldText, note, spaceaveraged);//Call method to performe the space average
	...
	public static void spaceAverage( short[][][] u,  short[][] mask, 
        ...
        double xcoeft = 0;
//		short mask[][] = new short[sizex+12][sizey+12];      //NEW moved to the begining
		//short uu[][][] = new short[sizex+12][sizey+12][ntimes];
