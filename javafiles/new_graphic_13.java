...
public 	class 	new_graphic_13
	...
	public static void main(String args[]) {
		...
		new_graphic_13 a = new new_graphic_13();
	...
	public void paint(Graphics g) {
		...
		if (startmovie == false) {
			...
			if (savemaskpng==1) {
				...
				/*
				try {
					String maskfilenamepng2 = String.valueOf("mask/0"+22+".png");
					DataOutputStream out = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(maskfilenamepng2)));
					System.out.println("mask lg OUT "+mask.length+" "+mask[0].length);
					for (int x = 0; x < mask.length; x++) {
						for (int y = 0; y < mask[0].length; y++) {
							out.writeShort(mask[x][y]);
							if (x==25 && y>=25 && y<=60) {
								System.out.println("mask OUT "+mask[x][y]+"  (x "+x+", y "+y+")");
							}
						}
					}
					out.close();
				...
				*/
				/*try {
					String maskfilenamepng2 = String.valueOf("mask/0"+22+".png");
					OutputStream out = new FileOutputStream(maskfilenamepng2);
					System.out.println("mask lg OUT "+mask.length+" "+mask[0].length);
					for (int x = 0; x < mask.length; x++) {
						for (int y = 0; y < mask[0].length; y++) {
							out.write(mask[x][y]);
	...
	//*************************************************************
		...
		if (ae.getSource() == loadmask) {
			...
			Image imageloadmask = toolkit.getImage(maskname);
			tracker.addImage(imageloadmask, 0);
			try {
				tracker.waitForAll();
			} catch (InterruptedException ex) {
			}
			
			width = imageloadmask.getWidth(this);
			height = imageloadmask.getHeight(this);
			System.out.println("width "+width+" height "+height);
			
			bimask = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
			big = bimask.getGraphics();
			big.drawImage(imageloadmask, 0, 0, this);
			*/
			...
			for (int x = 0; x < width; x++) {
				for (int y = 0; y < height; y++){
					mask[x][y]=(short)(imageloadmask.getRGB(x,y));
				}
			}*/
			/*
			...
			try {
				...
				int maskx = imageloadmask.getWidth();
				int masky = imageloadmask.getHeight();
			...
			*/
			...
			try{
				...
				byte[] b = new byte[24];
				is.read(b);
				System.out.println("mask lg IN "+mask.length+" "+mask[0].length);
				for (int x = 0; x < mask.length-1; x++) {
					for (int y = 0; y < mask[0].length-1; y++) {
						mask[x][y] = (short)(is.readShort());
						if (x==25 && y>=25 && y<=60) {
							System.out.println("mask IN "+mask[x][y]+"  (x "+x+", y "+y+")");
						}
					}
				}
			}
