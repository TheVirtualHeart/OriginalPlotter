...
/*
         <applet code="fourv0dNNN" width=450 height=400>
         </applet>
 */

public class fourv0dNNN extends Applet 
...
implements   ActionListener, ItemListener {
	...
	public void init() {
		...
		if(itype==3){
			...
			endtime=xnstims2+700; } // changed to match longer time NEWWWWWWWWW
...
//		checkvoltold= new Checkbox("Old Voltage", false);
//		checkvold = new Checkbox("Old v gate", false);
//		checkwold = new Checkbox("Old w gate", false);
//		checksold = new Checkbox("Old s gate", false);
...
//		checkvoltold.addItemListener(this); 
//		checkvold.addItemListener(this); 
//		checkwold.addItemListener(this); 
//		checksold.addItemListener(this); 
...
//		checkPanel.add(checkvoltold);
//		checkPanel.add(checkvold);
//		checkPanel.add(checkwold);
//		checkPanel.add(checksold);

//                  checkvoltold.setEnabled(false);
//                  checkvold.setEnabled(false);
//                  checkwold.setEnabled(false);
...
//				Tbutton7b.setText("0.9087");
			...
			if(itype==3){
				...
				endtime=xnstims2+700; } // changed to match longer time NEWWWWWWW
			...
			for(int ntime=1; ntime<nend; ntime++){
				...
				if(ncount==skip){
					...
					/*
					 * Added itype4 because I believe when switched between
					 * itype(1-3) and itype4 we change these values.  We never
					 * did in itype4
					 */
					if(itype == 4){
						if(u > theta_o){
...
//                   if(itype==4){
//                  checkvoltold.setEnabled(true);
//                  checkvold.setEnabled(true);
//                  checkwold.setEnabled(true);
...
//                                 }
	...
	public void  actionPerformed( ActionEvent ae)
	{
		...
		if(itype==3){
			...
			endtime=xnstims2+700; } // changed time to match longer one NEWWWWWWWWWWWWWWWWWWW
		nend=(int)(endtime/dt);
		ttime=0;
		...
		if(ae.getSource()==butt){
			...
			if(butt.getLabel().equals("Start")){
				...
				if(itype==3){
					...
					endtime=xnstims2+700; }  // changed time to match longer one NEWWWWWWWWWWWWW
				...
				if(itype==3){
					...
					endtime=xnstims2+700; } // changed to match the longer time NEWWW
