...
/*---- define gloval -----*/
  ...
  double fffx[] = new double[NN];  /* i-th x-force extra    */
  double fffy[] = new double[NN];  /* i-th y-force extra   */
...
//          }
          ...
          iyy = (int)(sc*yy[j])+gby;
               ...
               fffx[i]=0;
            if(p !=null){
                if( ix> (p.x-30)-15+27 && ix < (p.x-30)+15+27 && iy> (p.y-60)-15+60 && iy < (p.y-60)+15+60   ){
                gOff.setColor(Color.red);
                fffy[i]=20;
                fffx[i]=8;
           
                                                 }
        //        if( iy> (p.y-60)-15+60 && iy < (p.y-60)+15+60 ){
        //        gOff.setColor(Color.red);
        //                                         }
    ...
    gOff.drawString("mouse press"+(sc*xx[10])+gbx,630/6*3+10,40);
    gOff.drawString("mouse press"+(sc*yy[10])+gby,630/6*3+10,60);

    if(p !=null)gOff.drawString("mouse press"+(p.x-30.),630/6*3+10,90);
    if(p !=null)gOff.drawString("mouse press"+(p.y-60.),630/6*3+10,120);
...
/* This is to make the smaller rectangle tissue  */        
           if(y+x*.25>0.04 && y+x*.25<0.15 && ((x-.5)*(x-.5))<.02*.02) {
          xx[Nmt] = x;
          yy[Nmt] = y;
          kind[Nmt] = 0;
          mm[Nmt] = 0.01;
