...
/*      <applet code="alternansmallNielsL" width=870 height=650>
         </applet>
*/

	public class alternansmallNielsL extends Applet
	implements MouseListener, ActionListener, ItemListener, Runnable{
        Point p;
         private Panel buttonPanel, checkPanel, stimPanel;

//        Label label;
        Button butt,butf,buts,butr,butrs,butla,butsm,butstop,butres,butc;
        Image Buffer;
        Checkbox chplot;
	...
	boolean down;
        ...
        double dapd0,dapdl,dapd0g,dapdlg;
        double dbcln=0;
        double dbcln1=0;
        double ddi0;
        double ddil;
        int recalc=1;
        int controlcounter0=0;
        int controlcounterl=0;
...
///-------for control---------------------
        double bcl0=165;
        double edi=(bcl0-66)/2.05;
        double epad=1.05*edi+66;
        double a00= 46.316; 
        double a11 = 10.923; 
        double a22 = 28.9574; 
        double ecv=a00 - a11*Math.exp(-edi/a22);
        double  APDprime = 1.05;
        double cvprime = a11/(0.001*a22)*Math.exp(-edi/a22); 
        double cvinv_prime = -cvprime/(ecv*ecv); 
        double the_exp,AA,BB,CC,DD,det,ainv,binv,cinv,dinv; 
        double LL;
        double cgama=-1.05;
        double niels=.40;
...
//         ib[i]=255-(int)(1.5*n);
           LL=nnx*dx;
           the_exp = Math.exp(cvinv_prime*LL);
           AA = - APDprime;
           BB = 1;
           CC = - the_exp * ( APDprime + LL*(1+APDprime)*cvinv_prime );
           DD = the_exp;
          det = AA*DD - BB*CC;
          ainv = DD/det;
          binv = - BB/det;
...
//        options.addItem("gamma 1");
	...
	butc = new Button ("Cont L");
        butc.addActionListener( this );
        buttonPanel.add (butc);
...
//                                                }
           ...
           if(ae.getSource()==butc){
                      if(butc.getLabel().equals("Cont L")){
                         butc.setLabel("Cont L/2");
                         nbutc=0;
                         nnx= nx/2;                             }
                      else{
                         butc.setLabel("Cont L");
                         nbutc=1;
                         nnx=nx;
...
//         gBuffer.fillRect(415,70,200,100);
        ...
        gBuffer.setColor(Color.white);
         gBuffer.fillRect(415,50,500,250);
...
//        gBuffer.drawString("120--",15,330);
        ...
        gBuffer.drawString("APD0old="+String.valueOf(apd0old),585,80);
        gBuffer.drawString("APD0new="+String.valueOf(apd0new),585,90);
        gBuffer.drawString("APD0dif="+String.valueOf(dapd0),585,100);

        gBuffer.drawString("APDlold="+String.valueOf(apdlold),585,120);
        gBuffer.drawString("APDlnew="+String.valueOf(apdlnew),585,130);
        gBuffer.drawString("APDldif="+String.valueOf(dapdl),585,140);
//        gBuffer.drawString("APDldif)="+String.valueOf(ntpc[0]),585,140);

        gBuffer.drawString("ai="+String.valueOf(ainv),585,160);
        gBuffer.drawString("bi="+String.valueOf(binv),585,170);
        gBuffer.drawString("ci="+String.valueOf(cinv),585,180);
        gBuffer.drawString("di="+String.valueOf(dinv),585,190);
        gBuffer.drawString("L="+String.valueOf(LL),585,200);
        gBuffer.drawString("counter0="+String.valueOf(controlcounter0),585,210);
        gBuffer.drawString("counterl="+String.valueOf(controlcounterl),585,220);

        gBuffer.drawString("bcln="+String.valueOf(dbcln),585,230);
        gBuffer.drawString("bcln1="+String.valueOf(dbcln1),585,240);

        gBuffer.drawString("deltap="+String.valueOf(deltap/niels),585,250);
        gBuffer.drawString("ideltap="+String.valueOf(ideltap),585,260);
        gBuffer.drawString("recalc="+String.valueOf(recalc),585,270);

        gBuffer.drawString("nbutc="+String.valueOf(nbutc),585,280);
        gBuffer.drawString("nnx="+String.valueOf(nnx),585,290);
...
//            gBuffer.setColor(Color.black); }
  ...
  //            if(deltap < 0 )deltapp=deltap;
             //  if(nbutc==1){ideltap=(int)(deltap/dt);}
             //   else{ ideltap=0;}
                ideltap=(int)(deltap/dt);

             if(nppc==npp+ideltap || ifirst == 20){
             //  ifirst=1;
              u[1]=1;
              u[2]=1;
              u[3]=1;
...
//            if((u[23]-.25)*(ut[23]-.25) < 0.  ){
//                ntpc[0]=ntpc[0]+1;
//                 tpc[0][ntpc[0]]=ntime*dt-((-.25+u[23])/(ut[23]-u[23]))*dt;
//                if(ntpc[0]==2){ 
//                apd0old=apd0new;
//                apd0new=tpc[0][2]-tpc[0][1];
//                dapd0=apd0new-apd0old;
//                             }
//                 if(ntpc[0]==3){ ntpc[0]=1;
//                                 tpc[0][1]=tpc[0][3];
//                               }
//                                            }
//
//            if((u[96]-.25)*(ut[96]-.25) < 0. && napd[96]==1  ){
//                 ntpc[1]=ntpc[1]+1;
//                 tpc[1][ntpc[1]]=ntime*dt-((-.25+u[96])/(ut[96]-u[96]))*dt;
//                if(ntpc[1]==2){ 
//                apdlold=apdlnew;
//                apdlnew=tpc[1][2]-tpc[1][1];
//                dapdl=apdlnew-apdlold;
//                             }
//                 if(ntpc[1]==3){ ntpc[1]=1;
 //                                tpc[1][1]=tpc[1][3];
 //                              }
 //                                           }
...
//                              }
                ...
                apd0new=lapd1[6];
                dapd0=apd0new-apd0old;
                controlcounter0=controlcounter0+1;

//                dapd0g=dapd0/(1-cgama);
                ...
                apdlnew=lapd1[nnx-1];
                dapdl=apdlnew-apdlold;
                controlcounterl=controlcounterl+1;
//                dapdlg=dapdl/(1-cgama);
                         }
         ...
         int xntime2=(int)((nntime+1)/35);
          ...
          ncounter=0;
            ...
            nu[j][np]=u[j];
              ...
              if(controlcounterl > 14){
           if(controlcounterl-(controlcounterl/4)*4==1 ){ // && recalc==1){
                 recalc=0;
                 deltap=0;
                dapd0g=dapd0/(1-cgama);
                dapdlg=dapdl/(1-cgama);
                ddi0=-cgama*cgama*cgama*(dapd0g/APDprime);
                ddil=-cgama*cgama*cgama*(dapdlg/APDprime);
                dbcln=ainv*ddi0+binv*ddil;
                dbcln1=cinv*ddi0+dinv*ddil;
                                  }

             if(controlcounter0-4*(controlcounter0/4)==2){
                 deltap=dbcln*niels;     }
             if(controlcounter0-4*(controlcounter0/4)==3){
                 deltap=dbcln1*niels;     }

//            if(controlcounter0-4*(controlcounter0/4)==0){
//                deltap=0;   
//                  }
//            if(controlcounter0-4*(controlcounter0/4)==1){
//                deltap=0;     }


//            if(controlcounterl-4*(controlcounterl/4)==0){
//                recalc=1;  }
