...
//        if(y1<300)g.drawLine(ixx+60,y1,ixx+61,y2);
     ...
     if(i==0){ g.drawLine(ix,250,ix,y1);  
      ...
      if(i==1){ if(y1 > 310) y1=310;
        ...
        g.setColor(Color.orange);
        if(aapd1==aapd2){
        g.drawString("APD="+String.valueOf((aapd1))+"ms",315,58);
                         }
         else{
        g.drawString("APD_1="+String.valueOf((aapd1))+"ms",315,58);
        g.drawString("APD_2="+String.valueOf((aapd2))+"ms",315,78);
...
//              g.drawLine(per-150+200,552,per-150+200,300);
//          g.drawLine(pi.x,552,pi.x,320);
...
//         fppi=a0+a1*ppi+a2*ppi*ppi+a3*ppi*ppi*ppi+a4*ppi*ppi*ppi*ppi;
    ...
    //          g.drawOval(j-150,555-apds2[j],2,2);
          ...
          if(sen<1){
            ...
            iapd2=(int)(apd2/1.5);
              g.setColor(Color.red);
   //           g.fillOval(per-150,554-iapd1,10,10);
   //           g.fillOval(per-150,554-iapd2,10,10);
      //        g.setColor(Color.green);
      //        g.drawLine(per-150,552,per-150,300);
                     }
   //           g.setColor(Color.red);
