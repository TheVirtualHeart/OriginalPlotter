...
/*      <applet code="grayscott1" width=250 height=280>
         </applet>
*/

	public class grayscott1 extends Applet
...
//         g.drawLine(10,10,p.x,p.y);
         ...
         double x=0.0;
           ...
           for (int i=0; i<nx-1; i++) {
              du=(a*u[i]*u[i]*v[i]-u[i])/a1;
              dv=-v[i]*(1+u[i]*u[i])+1;
              if(i==0) {
                 xlapu=u[i+1]-2.*u[i]+u[i+1];
                 xlapv=v[i+1]-2.*v[i]+v[i+1];
              }
              else if(i==nx-1) {
                 xlapu=u[i-1]-2.*u[i]+u[i-1];
                 xlapv=v[i-1]-2.*v[i]+v[i-1];
              ...
              ut[i]=u[i]+dt*(dd*xlapu/a1+du);
              vt[i]=v[i]+dt*(h2*xlapv+dv);
           }
