 ...
 */
 ...
 /*
 To compile it needs:
 - DIRECTORIES: "/movie", "/mask", "/analyzed", "/info"
...
public 	class 	new_graphic_20
	...
	public static void main(String args[]) {
		...
		new_graphic_20 a = new new_graphic_20();
		
	}//close main
	//**************************************************************************	
	//This method builts the graphical interface		
	public new_graphic_20() {
		...
		//INTERFACE ------------------------------------------------------------
		{
			...
			saveapdspace= new Button("Save APDsp");
			savedispace = new Button("Save DIsp");
			// ----
			...
			// Elements Position -----------------------------------------------
			{
				...
				rowW.add	( saveapdspace );
				rowW.add	( savedispace );
			...
			if (apdDiMeanc) {
				...
				if (mapdAo[nii][njj]<=maxapd && mdiAo[nii][njj]<=maxdi) {
					if (alternancec) {
						...
						// Save APD - DI mean value without alternance
						if (saveapddimeanvalue==1) {
							String s = "Mean APD - DI without alternance\n";
	...
	//**************************************************************************
		...
		if (ae.getSource() == start && start.getLabel().equals("Start New")) {
			...
			if (ae.getSource() == start && start.getLabel().equals("Reset")) {
				...
				diSpacec=false; frame11=false;frame1p=false;
				// Reset to plot
				mintimeplot = 10;
		...
		if (ae.getSource() == setshift) {
			...
			shifted = copyMatrix3D(tempshifted,sizex,sizey,ntimes); tempshifted = null;
			basevL = copyMatrix3D(tempbasevL,sizex,sizey,ntimes); tempbasevL = null;;
			intervsh1f = copyMatrix2D(intervsh1first,sizex,sizey); intervsh1first = null;
			intervsh2p = copyMatrix2D(intervsh2final,sizex,sizey); intervsh2final = null;
		...
		if (ae.getSource() == generateS) {
			...
			if (generate == 7) { // Space averaged
				...
				for(int t = mintimeplot; t < maxtimeplot; t = t + dt) {
					...
					int tT1 = t/dt;
					datat[jjj]=(float)(tT1);
					data[jjj]=(float)(spaceaveraged[nii][njj][tT1]);
				}
			...
			if (generate == 10) { // APD - DI mean value
				...
				if (alternancec) {
					...
					jjj=jjj+1;
					datat[jjj]=(float)(dt*meandi[nii][njj]);
					data[jjj]=(float)(dt*meanapd[nii][njj]);
				}
