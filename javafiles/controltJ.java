...
/*      <applet code="controltJ" width=450 height=630>
         </applet>
*/

	public class controltJ extends Applet
...
//                                                }
           ...
           if(ae.getSource()==butstop){
                      if(butstop.getLabel().equals("excite T")){
                       butstop.setLabel(" Stop ");
                      gamaq=0;
                     //  nstop=1;
                                                        }
                       else{
                       butstop.setLabel("excite T");
                       gamaq=gama;
                      // nstop=10;
                            } 
                                }
...
//
	public void update(Graphics g){ paint(g); }  
        ...
        gBuffer.drawString("L= ("+String.valueOf(deltat)+") points",20,320);
        gBuffer.drawString("T= "+String.valueOf(gamaq),130,320);
        gBuffer.drawString("T= "+String.valueOf(iperiod)+"ms",130,332);
        gBuffer.drawString("nx= "+String.valueOf(nx)+"ms",190,332);
...
//                                                                        }
         if(gamaq > 0 ) gBuffer.drawLine(2*nbu1+20,69,2*nbu1+20,478);
...
//              if(ut[j] < nu[j][nppc] && j==1){
//              if(ut[nx-2] < nu[nx-2][nppc] && j==8){

              if(j==nx-8){

              ut[j]=ut[j]-gama*(u[nx-8]-nu[nx-8][npc])*dt;
                                      }
            //           }
            ...
            if((u[j]-.25)*(ut[j]-.25) < 0. && napd[j]==1 ){
             ...
             tpa[j][ntpa[j]]=ntime*dt-((-.25+u[j])/(ut[j]-u[j]))*dt;
                ...
                if(ntpa[j]==2){
                 ...
                 if(j==nbu1){lapdold=lapdn; 
                             lapdn=lapd1[j];
                          
                            deltat=gamaq*(lapdn-lapdold)/dt;
                           if(gamaq==0)deltat=0;
                       //     deltat=0*(lapdn-lapdold)/dt;
