...
public 	class 	new_graphic_05
	extends	Frame   implements MouseListener, ActionListener, ItemListener, Runnable {
	
	//Interface declarations
	private Button		start, stop, pause, save, movie;
...
//	int dpx		= 130;		//Distance from x window boundary
	...
	int dsy		= dpy + phm + 15;	//Distance from graph boundary	
   ...
   //     Color background = new Color(102,205,170);
        Color background = new Color(46,139,87);
...
//        setBackground(background);
	...
	public static void main(String args[]) {
		...
		//Create an instance of new_graphic_05_up 
		new_graphic_05 a = new new_graphic_05();
	...
	thrset		= new Button("Set");
       ...
       // horiz=new Scrollbar(Scrollbar.VERTICAL,0,10,1,500);
       // add(horiz);
     //   horiz.reshape(285,385,200,15);
     //   prompthoriz=new Label ("Movie Speed");
     //   Font font1 = new Font("Helvetica", Font.BOLD, 15);
     //   prompthoriz.setFont(font1);
     //     add(prompthoriz);
     //      prompthoriz.reshape(305,400,200,14);
	...
	public void paint(Graphics g) {
		...
		if (startmovie == false) {
			...
			if (frame11) {
				...
				//Color table and first frame
				{
				
					int ftime = 10;  //frame choosen to show the structure.
				/*
					if (nmap == 0) {
						for (int ii = 0; ii < maplength; ii += 2){
							...
							gBuffer.fillRect(dsx + 2*ss + 10, dsy + ss - ii/2 + 1, 10, 2);
						}
						for (int i = 0; i < ss; i++) {
							for (int j = 0; j < ss; j++) {
								int iii=(int)(190*taveraged[i][j][ftime]/norma);
								if(iii <30) iii=30;
								gBuffer.setColor(new Color(ir[iii], ig[iii], ib[iii]));
					...
					if (nmap == 1) {
						for (int ii = 0; ii < maplength; ii += 2){
							...
							gBuffer.fillRect(dsx + 2*ss + 10, dsy + ss - ii/2 + 1, 10, 1);
						}
						for (int i = 0; i < ss; i++) {
							for (int j = 0; j < ss; j++) {
								int iii=(int)(190*taveraged[i][j][ftime]/norma);
				...
				//	}
					...
					gBuffer.setColor(Color.black);
					gBuffer.drawString(" V max", dsx + 2*ss + 22, dsy + ss - maplength/2 + 10);
					gBuffer.drawString(" V min", dsx + 2*ss + 22, dsy + ss - 2);
				}
			...
			if (apdDistc) {
				...
				gBuffer.drawString("DI (ms)", dpx + pw/2, phG + dpy - day/3);
				gBuffer.drawString("APD (ms)", dpx + dax/4, dpy + day/2 + 2);
				
				gBuffer.setColor(Color.yellow);
				gBuffer.drawString("APD - DI distribution points", dpx + dax + pw/4, dpy + 20);
				...
				gBuffer.drawString(Integer.toString(mindi), dpx + dax, dpy + phG - day/2);
				gBuffer.drawString(Integer.toString(maxdi), dpx + pw - dax - dax/2, dpy + phG - day/2);
				gBuffer.drawString(Integer.toString(minapd), dpx + dax/3, dpy + phG - day - 1);
				gBuffer.drawString(Integer.toString(maxapd), dpx + dax/3, dpy + day + 10);
			...
			if (apdDiMeanc) {
				...
				gBuffer.setColor(Color.green);
				gBuffer.drawString("APD - DI restitution curve", dpx + dax + pw/4, dpy + 20);
			...
			// ----------------- ----------- -------------------
                          ...
                          //---for one frame of the stucture--- Alessio look here--

                          //	for (int i = 0; i < ss; i++) {
			  //     	for (int j = 0; j < ss; j++) {
                          //      int iii=(int)(original[i][j][10]);
			//	gBuffer.setColor(new Color(irrr[iii], iggg[iii], ibbb[iii]));
			//	gBuffer.fillRect(dsx+i*2 , dsy+j*2 , 2, 2);
                          //                                                     }
                           //                                                    }                
		...
		//	g.fillRect(dsx, dsy, ss + 4, ss + 4);
			...
			*/
			...
			//Create image
                       // g.setColor(Color.white);
                       // g.fillRect(dsx + ss + 102, dsy + ss - maplength/2 + 30, 6, 6);
			gBuffer.setColor(Color.black);
			gBuffer.drawString(" time = ", dsx + 2*ss + 42, dsy + ss - maplength/2 + 60);
			gBuffer.drawString(String.valueOf(time*2)+" ms", dsx + 2*ss + 85, dsy + ss - maplength/2 + 60 );
	...
	//		repaint(dsx + 2, dsy + 2, ss+150, ss+150);
