...
/*      <applet code="brineN" width=800 height=800>
         </applet>
*/
public class brineN extends Applet implements Runnable {
  int x = 150, y = 150, r=70;      // Position and radius of the circle
  int dx = 3, dy = 3, dyo, dxo;              // Trajectory of circle
  int n=1;
  int m=1;
  ...
  public void run() {
    while(!please_stop) {
     ...
     m=m+1;
       ...
       if(n==4){
       dangle=.5*m_r.nextGaussian();
       angle=angle+dangle;
       
       dxo=dx;
	dyo=dy;
        dx=(int)(8*Math.cos(angle));
	dy=(int)(8*Math.sin(angle));    //Math.random();
...
//		     }
	...
	n=0;
      ...
      // Move the circle.
      x += dx;  y += dy;
      if ((x    < 20) || (x  > size.width-38)) x = x-dx;
      if ((y    < 8) || (y  > size.height-98)) y = y-dy;
...
//      repaint(x-r, y-r, 2*r, 2*r);         // repaint new position of circle
      ...
      try {
//         Thread.sleep(1);
      if(m==1)Thread.sleep(290);
      if(m==2)Thread.sleep(90);
