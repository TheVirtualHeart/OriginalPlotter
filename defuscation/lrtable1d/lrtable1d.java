//    Java program for Luo-Rudy cell model in 1D
//
// Copyright (c) 2004 Flavio H Fenton.
//
//
//
//
...
// Also, if possible send an e-mail to fenton@presto.physics.neu.edu whenever changes or 
...
//*******************************************************************************
	...
	import java.awt.event.*;
        import java.lang.*;
         ...
         /*
         <applet code="lrtable1d" width=600 height=450>
         </applet>
          */

	public class lrtable1d extends Applet 
...
//    8 variables of the model
        double x1[]= new double[1301];
        double m[]= new double[1301];
        double h[]= new double[1301];
        double j[]= new double[1301];
        double d[]= new double[1301];
        double f[]= new double[1301];
        double u[]= new double[1301];
        double ca[]= new double[1301];
        double ut[]= new double[1301];
        int nx=501;
        int mx=500;
        double xca;
//          
        ...
        double xik1,xik2,bk1,ak1,ikp,ib,ikk;
        double gna=23.0;
        double gnac=0.003;
        double ena=54.4;
        double gs=0.09; 
        ...
        double ax1t[] = new double[36000];
        double bx1t[] = new double[36000];
        double amt[] = new double[36000];
        double bmt[] = new double[36000];
        double aht[] = new double[36000];
        double bht[] = new double[36000];
        double ajt[] = new double[36000];
        double bjt[] = new double[36000];
        double adt[] = new double[36000];
        double bdt[] = new double[36000];
        double aft[] = new double[36000];
        double bft[] = new double[36000];
        double ik1t[] = new double[36000];
//        double gix1t[] = new double[36000];
        double xik1t[] = new double[36000];
        double ikpt[] = new double[36000];
...
//   ******************TABLE**************** 
      for(int i=0; i< 36000; i++){
       ...
       if(xu>=-40){
         ...
         bjt[i]=0.3*Math.exp(-0.0000002535*xu)/(1.0+Math.exp(-0.1*(xu+32)));
                 }
         else{
         ...
         double amtt=(1.-Math.exp(-0.1*(xu+47.13)));
       if(amtt==0){
        amt[i]=amt[i-1];
                  }
          else{
        ...
        ax1t[i]=0.0005*Math.exp(0.083*(xu+50))/(1.+Math.exp(0.057*(xu+50)));
        bx1t[i]=0.0013*Math.exp(-0.06*(xu+20))/(1.+Math.exp(-0.04*(xu+20)));

      double xikk1t=((xu+77)*Math.exp(0.04*(xu+35)));
        if(xikk1t==0){xik1t[i]=xik1t[i-1];}
         else{
       xik1t[i]=2.837*(Math.exp(0.04*(xu+77.))-1.)/((xu+77)*Math.exp(0.04*(xu+35)));
             }
        
        ak1=1.02/(1.+Math.exp(0.2385*(xu-ek1-59.215)));
        bk1=(0.49124*Math.exp(0.08032*(xu-ek1+5.476))+Math.exp(0.06175*(xu-ek1-594.31)))/(1.+Math.exp(-0.5143*(xu-ek1+4.753)));
       if(ak1+bk1==0){ik1t[i]=ik1t[i-1];}
        else{
       ik1t[i]=(ak1/(ak1+bk1))*(xu-ek1);
            }
...
//        buttonPanel = new Panel();
	...
	slider1 = new Scrollbar(Scrollbar.HORIZONTAL, 500, 60, 20, 1360);
        add(slider1);
...
//           add(buttonPanel, BorderLayout.NORTH);
	...
	public void paint(Graphics g) {
        if(firsttime){
          ...
          gBuffer.drawString("-90",2,240);
          gBuffer.drawString("mV",1,140);
          gBuffer.drawString("15",2,50);
...
//         ix1=gix1t[ntab]*x1[ii];
         ina=(gna*m[ii]*m[ii]*m[ii]*h[ii]*j[ii])*(u[ii]-ena);
         xlap=u[ii-1]+u[ii+1]-2.*u[ii];


         ib=0.03921*(u[ii]+59.87);
         ikk=gkk*x1[ii]*xik1t[ntab]*(u[ii]+77.56);  
         ik1=gk1*ik1t[ntab];


        ut[ii]=u[ii]-dt*(ina+is+ikk+ik1+ikpt[ntab]+ib+stimsenx*Istim)+Dlap*dt*xlap/dx2;
...
//         ix1=gix1t[ntab]*x1[0];
         ina=(gna*m[0]*m[0]*m[0]*h[0]*j[0])*(u[0]-ena);
         if(ncable==1){
         xlap=2.*u[1]-2.*u[0];
                      }
         else{
         ...
         ib=0.03921*(u[0]+59.87);
         ikk=gkk*x1[0]*xik1t[ntab]*(u[0]+77.56);  
         ik1=gk1*ik1t[ntab];


        ut[0]=u[0]-dt*(ina+is+ikk+ik1+ikpt[ntab]+ib+stimsenx*Istim)+Dlap*dt*xlap/dx2;
...
//         ix1=gix1t[ntab]*x1[mx];
         ina=(gna*m[mx]*m[mx]*m[mx]*h[mx]*j[mx])*(u[mx]-ena);
    if(ncable==1){
         ...
         ib=0.03921*(u[mx]+59.87);
         ikk=gkk*x1[mx]*xik1t[ntab]*(u[mx]+77.56);  
         ik1=gk1*ik1t[ntab];


        ut[mx]=u[mx]-dt*(ina+is+ikk+ik1+ikpt[ntab]+ib+stimsenx*Istim)+Dlap*dt*xlap/dx2;
...
//            a= Double.valueOf(input1.getText()).doubleValue();
            ...
            gk1= Double.valueOf(input3.getText()).doubleValue();
